...
interface AuthContextValue {
  ...
  signInWithGoogleToken: (token:string)=>Promise<AuthResult>;
...
export function AuthProvider({children}:{children:ReactNode}){
 ...
 const signInWithGoogleToken=useCallback(async(token:string)=>{const {data,error}=await supabase.auth.signInWithIdToken({provider:'google',token});if(error)return{error:friendly(error)};const nextProfile=data.user?await loadProfile(data.user.id):null;setSession(data.session);setUser(data.user);setProfile(nextProfile);return{error:null,role:nextProfile?.role??'customer'}},[loadProfile]);
 ...
 const role=profile?.role??'customer';const value=useMemo(()=>({user,session,profile,loading,roles:user?[role]:[],isAdmin:role==='admin',isStaff:role==='admin'||role==='manager',signUp,signIn,signInWithGoogleToken,resetPassword,signOut,refreshProfile}),[user,session,profile,loading,role,signUp,signIn,signInWithGoogleToken,resetPassword,signOut,refreshProfile]);
