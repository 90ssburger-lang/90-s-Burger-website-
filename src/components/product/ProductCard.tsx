...
export function ProductCard({ product }: { product: Product; featured?: boolean }) {
  ...
  return <article className="group flex h-full flex-col overflow-hidden rounded-3xl border border-black/10 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-xl">
    <Link to={`/product/${product.slug}`} className="relative block aspect-[4/3] overflow-hidden bg-[#fff7df]">
      ...
      {product.is_featured && <span className="absolute left-3 top-3 rounded-full bg-[#f6b73c] px-3 py-1 text-xs font-black uppercase">Fan favorite</span>}
    </Link>
    <div className="flex flex-1 flex-col p-5">
      <p className="text-xs font-bold uppercase tracking-[.15em] text-[#ef3e2f]">{product.category?.name || '90’s Burger'}</p>
      ...
      <div className="mt-5 flex items-center justify-between gap-3"><span className="text-lg font-black">{formatCurrency(product.price)}</span><Button onClick={() => addItem(product)} disabled={product.stock === 0} className="rounded-full bg-[#ef3e2f] text-white hover:bg-[#d92d20]"><Plus/> Add to Cart</Button></div>
    </div>
  </article>;
}
