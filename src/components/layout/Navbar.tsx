...
export function Navbar() {
  ...
  const { user } = useAuth();
  return <header className="sticky top-0 z-50 border-b border-primary/15 bg-background/95 backdrop-blur">
    <nav className="container mx-auto flex h-18 items-center justify-between px-4 py-3">
      <Link to="/" onClick={()=>setOpen(false)} className="flex items-center gap-3" aria-label="90's Burger home"><img src="/images/90s-burger-logo.png" width="128" height="128" alt="90's Burger" className="h-12 w-12 rounded-xl border border-primary/10 object-cover sm:h-14 sm:w-14"/><span className="hidden text-xl font-black uppercase tracking-[-.03em] text-primary sm:block"><span className="text-red-700">90's</span> Burger</span></Link>
      <div className="hidden items-center gap-7 md:flex"><Link to="/shop" className="text-sm font-bold uppercase">Full Menu</Link>{links.map(l=><Link key={l.slug} to={`/shop?category=${l.slug}`} className="text-sm font-bold uppercase text-muted-foreground hover:text-primary">{l.label}</Link>)}</div>
