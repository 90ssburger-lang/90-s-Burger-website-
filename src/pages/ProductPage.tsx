...
export default function ProductPage(){
 ...
 {addons.length>0&&<div className="mt-8"><h2 className="text-lg font-black uppercase">Choose your sides &amp; extras</h2><div className="mt-3 grid grid-cols-2 gap-2">{addons.filter(x=>x.is_enabled).map(x=><button key={x.id} onClick={()=>toggle(x)} className={`flex items-center justify-between rounded-2xl border-2 p-3 text-left text-sm font-bold ${selected.some(a=>a.id===x.id)?'border-[#ef3e2f] bg-red-50':'border-black/10'}`}><span>{x.name}<small className="block font-normal text-muted-foreground">+{formatCurrency(Number(x.price))}</small></span>{selected.some(a=>a.id===x.id)&&<Check className="text-[#ef3e2f]"/>}</button>)}</div></div>}
