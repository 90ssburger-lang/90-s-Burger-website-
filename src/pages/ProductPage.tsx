...
export default function ProductPage(){
 ...
 {product.category?.slug!=='sides'&&sideProducts.length>0&&<div className="mt-8"><h2 className="text-lg font-black uppercase">Add a side</h2><div className="mt-3 grid grid-cols-2 gap-2">{sideProducts.map(side=><button key={side.id} type="button" onClick={()=>toggleSide(side)} className={`flex items-center justify-between gap-2 rounded-2xl border-2 p-3 text-left text-sm font-bold ${selectedSides.some(x=>x.id===side.id)?'border-[#f0008f] bg-fuchsia-50':'border-black/10'}`}><span>{side.name}<small className="block font-normal text-muted-foreground">+{formatCurrency(Number(side.price))}</small></span>{selectedSides.some(x=>x.id===side.id)&&<Check className="shrink-0 text-[#f0008f]"/>}</button>)}</div></div>}
